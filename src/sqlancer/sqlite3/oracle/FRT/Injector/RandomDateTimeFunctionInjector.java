...
public class RandomDateTimeFunctionInjector {
    ...
    private static final List<DateTimeFuncDef> DATE_FUNCS = Arrays.asList(
//            new DateTimeFuncDef("DATE", 1),
//            new DateTimeFuncDef("TIME", 1),
//            new DateTimeFuncDef("DATETIME", 1),
//            new DateTimeFuncDef("JULIANDAY", 1),
            new DateTimeFuncDef("UNIXEPOCH", 1),
//            new DateTimeFuncDef("STRFTIME", 2),
            new DateTimeFuncDef("TIMEDIFF", 2)
    );

    private static class DateTimeFuncDef {
        String name;
        int argCount;

        DateTimeFuncDef(String name, int argCount) {
            this.name = name;
    ...
    private static List<ColumnOccurrence> findColumnsWithAggScope(String expr, int baseOffset) {
        ...
        while (aggMatcher.find()) {
            int openPar = expr.indexOf('(', aggMatcher.start());
    ...
    /* ================= 生成额外参数 ================= */
    private static String generateExtraArg(DateTimeFuncDef func) {
        switch (func.name) {
            case "STRFTIME":
                return "'%Y-%m-%d'";
            case "TIMEDIFF":
    ...
    /* ================= 核心注入逻辑 ================= */
    public static String injectSmartDateTimeFunction(String sql, int maxInjectCount) {
        ...
        for (String item : selectItems) {
            ...
            for (ColumnOccurrence c : cols) {
                if (!c.insideAgg) injectableColumns.add(c);
            }
        }
        ...
        for (ColumnOccurrence c : toInject) {
            DateTimeFuncDef func = DATE_FUNCS.get(RANDOM.nextInt(DATE_FUNCS.size()));
            List<String> args = new ArrayList<>();

            if (func.argCount == 1) {
                args.add(c.column);
            } else if (func.argCount == 2) {
                if (func.name.equals("STRFTIME")) {
