...
/**
 * SQLite3LMOracle: 合并版逻辑短路 Oracle（扩展）
 * - 除了 AND/OR 交换律检测，还加入：
 * - min/max 交换律
 * - BETWEEN 展开等价
 * - IN 展开等价
 ...
 * - 比较操作的对偶等价（< vs NOT >=）
 * - coalesce 短路（如果存在）
 * - 仍保留 sleep 注入短路检测、时间统计、异常匹配、仅在缺陷时抛 AssertionError
 */
public class SQLite3AllINOneOracle implements TestOracle<SQLite3GlobalState> {
    ...
    public SQLite3AllINOneOracle(SQLite3GlobalState globalState) {
    ...
    public void check() throws SQLException {
        ...
        try {
            ...
            // 其他等价检测
            checkMinMax();
            checkBetweenExpansion();
            checkInExpansion();
            checkDoubleNot();
            checkComparisonSymmetry();
    ...
    // =============== 原有 AND/OR 检测（抽出子方法） ===============
    private void checkAndOr() throws SQLException {
        String selectA = generateNestedWhereSelectWithChecks(1);
        String selectB = generateNonNullSelectExpr();

        if (shouldIgnoreMinMax(selectA, selectB)) {
            return; // 遇到 DISTINCT + OFFSET 的情况，直接忽略
        }
    ...
    private boolean shouldIgnoreMinMax(String sqlA, String sqlB) {
        if (sqlA != null && sqlA.toUpperCase().contains("DISTINCT") && sqlA.toUpperCase().contains("OFFSET")) {
            return true;
        }
        if (sqlB != null && sqlB.toUpperCase().contains("DISTINCT") && sqlB.toUpperCase().contains("OFFSET")) {
            return true;
        }
        return false;
    }

    // ===== TEMPORARY IGNORE INT/FLOAT DIFFERENCE =====
// This method allows 0 vs 0.0, 1 vs 1.0 to be treated as equal
// Remove this method when int/float min/max defect is fixed
    private void checkMinMax() throws SQLException {

        // --- 生成 A / B ---
        String a = generateNonNullSelectExpr();
        String b = generateNonNullSelectExpr();

        if (shouldIgnoreMinMax(a, b)) {
            return; // 遇到 DISTINCT + OFFSET 的情况，直接忽略
        }

        MutatedResult rA = exec(a);
        MutatedResult rB = exec(b);

        String minAB = "SELECT min((" + trim(a) + "), (" + trim(b) + "))";
        String minBA = "SELECT min((" + trim(b) + "), (" + trim(a) + "))";
        String maxAB = "SELECT max((" + trim(a) + "), (" + trim(b) + "))";
        String maxBA = "SELECT max((" + trim(b) + "), (" + trim(a) + "))";

        MutatedResult rMinAB = exec(minAB);
        MutatedResult rMinBA = exec(minBA);
        MutatedResult rMaxAB = exec(maxAB);
        MutatedResult rMaxBA = exec(maxBA);

        String minABSleep = "SELECT min((" + trim(a) + "), (" + wrapWithSleepShortCircuit(trim(b)) + "))";
        String minBASleep = "SELECT min((" + trim(b) + "), (" + wrapWithSleepShortCircuit(trim(a)) + "))";
        MutatedResult rMinABSleep = exec(minABSleep);
        MutatedResult rMinBASleep = exec(minBASleep);

        // ========= 单个内部方法：忽略 int/float 的值比较 =========
        // （嵌套在 checkMinMax 方法内部）
        java.util.function.BiPredicate<Object, Object> eq = (x, y) -> {
            if (x == null && y == null) return true;
            if (x == null || y == null) return false;

            String sx = x.toString().trim();
            String sy = y.toString().trim();

            if (sx.equals(sy)) return true;
        ...
        // ========= min/max 结果必须来自 {A, B} =========
        java.util.function.Predicate<MutatedResult> fromAB = r ->
                eq.test(r.value, rA.value) || eq.test(r.value, rB.value);

        boolean minResultOriginInvalid = !fromAB.test(rMinAB) || !fromAB.test(rMinBA);
        boolean maxResultOriginInvalid = !fromAB.test(rMaxAB) || !fromAB.test(rMaxBA);

        // ========= 一致性检测（包含交换律） =========
        boolean inconsistent =
                !eq.test(rMinAB.value, rMinBA.value) ||
                        !eq.test(rMaxAB.value, rMaxBA.value) ||
                        (rMinABSleep.timeNs - rMinAB.timeNs) > SLOW_THRESHOLD_NS ||
                        (rMinBASleep.timeNs - rMinBA.timeNs) > SLOW_THRESHOLD_NS ||
                        minResultOriginInvalid ||
                        maxResultOriginInvalid;

        if (inconsistent) {
            StringBuilder msg = new StringBuilder();
            msg.append("\n===== MIN/MAX INCONSISTENCY =====\n\n");

            msg.append("A: ").append(trim(a)).append("\n").append(dumpMutated(rA)).append("\n");
            msg.append("B: ").append(trim(b)).append("\n").append(dumpMutated(rB)).append("\n");

            msg.append("min AB: ").append(minAB).append("\n");
            msg.append(dumpMutated(rMinAB));
            if (!fromAB.test(rMinAB)) msg.append("  --> MIN AB result is NOT one of original values!\n");

            msg.append("min BA: ").append(minBA).append("\n");
            msg.append(dumpMutated(rMinBA));
            if (!fromAB.test(rMinBA)) msg.append("  --> MIN BA result is NOT one of original values!\n");

            msg.append("max AB: ").append(maxAB).append("\n");
            msg.append(dumpMutated(rMaxAB));
            if (!fromAB.test(rMaxAB)) msg.append("  --> MAX AB result is NOT one of original values!\n");

            msg.append("max BA: ").append(maxBA).append("\n");
            msg.append(dumpMutated(rMaxBA));
            if (!fromAB.test(rMaxBA)) msg.append("  --> MAX BA result is NOT one of original values!\n");
    ...
    // =================== 新增检测：BETWEEN 展开 ===================
    private void checkBetweenExpansion() throws SQLException {
        // 使用列或字面量
        String sel = generateNestedWhereSelectWithChecks(1);
        // 取 WHERE 表达式作为 A (trim 去掉 SELECT ... WHERE )
        String whereExpr = extractWhereExpr(sel);
        if (whereExpr == null) return;

        // 构造 A BETWEEN L AND R
        String a = "(" + whereExpr + ")";
        String l = generateNonNullSelectExpr();
        String r = generateNonNullSelectExpr();

        String between = "SELECT (" + a + " BETWEEN (" + trim(l) + ") AND (" + trim(r) + "))";
        String expanded = "SELECT ((" + a + ") >= (" + trim(l) + ") AND ((" + a + ") <= (" + trim(r) + ")))";

        MutatedResult rb = exec(between);
        MutatedResult re = exec(expanded);

        if (mismatch(rb, re)) {

            StringBuilder msg = new StringBuilder();
            msg.append("\n===== BETWEEN EXPANSION INCONSISTENCY =====\n\n");
            msg.append("between: ").append(between).append("\n");
            msg.append(dumpMutated(rb)).append("\n");
            msg.append("expanded: ").append(expanded).append("\n");
            msg.append(dumpMutated(re)).append("\n");
            msg.append("\n------ SCHEMA + SAMPLE DATA ------\n")
                    .append(globalState.getSchema()).append("\n")
                    .append(dumpAllTables(globalState)).append("\n");
    ...
    // =================== 新增检测：IN 展开 ===================
    private void checkInExpansion() throws SQLException {
        // 生成左表达式和两个右侧表达式
        String left = generateNonNullSelectExpr();
        String v1 = generateNonNullSelectExpr();
        String v2 = generateNonNullSelectExpr();

        // 统一类型为 NUMERIC，避免 value-list IN 与 OR 展开不一致
        left = "CAST((" + trim(left) + ") AS NUMERIC)";
        v1 = "CAST((" + trim(v1) + ") AS NUMERIC)";
        v2 = "CAST((" + trim(v2) + ") AS NUMERIC)";

        // 生成 IN 表达式，右侧改为子查询形式，继承 affinity，避免 value-list 的问题
        String in = "SELECT (" + left + " IN (" +
                "SELECT " + v1 + " UNION ALL SELECT " + v2 +
                "))";

        // 展开 OR 时直接比较，不加 IS NOT NULL，保持逻辑一致
        String expanded = "SELECT ((" + left + " = " + v1 + ") OR (" + left + " = " + v2 + "))";

        // 执行 SQL
        MutatedResult rin = exec(in);
        MutatedResult rexp = exec(expanded);

        // 检查结果是否一致
        if (mismatch(rin, rexp)) {
            StringBuilder msg = new StringBuilder();
            msg.append("\n===== IN EXPANSION INCONSISTENCY =====\n\n");
            msg.append("in: ").append(in).append("\n");
            msg.append(dumpMutated(rin)).append("\n");
            msg.append("expanded: ").append(expanded).append("\n");
            msg.append(dumpMutated(rexp)).append("\n");
            msg.append("\n------ SCHEMA + SAMPLE DATA ------\n")
                    .append(globalState.getSchema()).append("\n")
                    .append(dumpAllTables(globalState)).append("\n");
    ...
    private void checkDoubleNot() throws SQLException {
        String sel = generateNonNullSelectExpr();
        String trimmedSel = trim(sel);

        String origSQL = "SELECT (" + trimmedSel + ")";
        String notnotSQL = "SELECT NOT NOT (" + trimmedSel + ")";

        MutatedResult ro = exec(origSQL);
        MutatedResult rnn = exec(notnotSQL);

        // New: determine boolean value via CASE WHEN ...
        String roCaseSQL = "SELECT CASE WHEN (" + trimmedSel + ") THEN 1 ELSE 0 END";
        String rnnCaseSQL = "SELECT CASE WHEN NOT NOT (" + trimmedSel + ") THEN 1 ELSE 0 END";

        MutatedResult roCase = exec(roCaseSQL);
        MutatedResult rnnCase = exec(rnnCaseSQL);

        Boolean roBool = "1".equals(roCase.value);
        Boolean rnnBool = "1".equals(rnnCase.value);

        if (!Objects.equals(roBool, rnnBool)) {
            StringBuilder msg = new StringBuilder();
            msg.append("\n===== DOUBLE NOT INCONSISTENCY =====\n\n");
            msg.append("orig: ").append(origSQL).append("\n");
            msg.append("orig value: ").append(ro.value).append("\n");
            msg.append("orig bool via CASE: ").append(roBool).append("\n");
            msg.append("notnot: ").append(notnotSQL).append("\n");
            msg.append("notnot value: ").append(rnn.value).append("\n");
            msg.append("notnot bool via CASE: ").append(rnnBool).append("\n");
            msg.append("\n------ SCHEMA + SAMPLE DATA ------\n")
                    .append(globalState.getSchema()).append("\n")
                    .append(dumpAllTables(globalState)).append("\n");
    ...
    // =================== 新增检测：比较对偶（< vs NOT >=） ===================
    private void checkComparisonSymmetry() throws SQLException {
        String a = generateNonNullSelectExpr();
        String b = generateNonNullSelectExpr();

        String lt = "SELECT (" + trim(a) + " < " + trim(b) + ")";
        String notGe = "SELECT NOT (" + trim(b) + " >= " + trim(a) + ")";

        MutatedResult rlt = exec(lt);
        MutatedResult rng = exec(notGe);

        if (mismatch(rlt, rng)) {

            StringBuilder msg = new StringBuilder();
            msg.append("\n===== COMPARISON SYMMETRY INCONSISTENCY =====\n\n");
            msg.append("lt: ").append(lt).append("\n");
            msg.append(dumpMutated(rlt)).append("\n");
            msg.append("notGe: ").append(notGe).append("\n");
            msg.append(dumpMutated(rng)).append("\n");
            msg.append("\n------ SCHEMA + SAMPLE DATA ------\n")
                    .append(globalState.getSchema()).append("\n")
                    .append(dumpAllTables(globalState)).append("\n");
    ...
    private String generateNestedWhereSelectWithChecks(int maxDepth) throws SQLException {
        ...
        SQLite3Expression whereExpr = generateNestedExpressionWithChecks(1);
    ...
    private String extractWhereExpr(String select) {
        if (select == null) return null;
        String lower = select.toLowerCase();
        int where = lower.indexOf(" where ");
        if (where < 0) return null;
        String after = select.substring(where + 7);
        // 可能包含 LIMIT、ORDER BY 等，简化：只取到末尾或遇到 " limit ", " order ", ";"
        int end = after.length();
        String lowAfter = after.toLowerCase();
        int lim = lowAfter.indexOf(" limit ");
        if (lim >= 0) end = Math.min(end, lim);
        int ord = lowAfter.indexOf(" order ");
        if (ord >= 0) end = Math.min(end, ord);
