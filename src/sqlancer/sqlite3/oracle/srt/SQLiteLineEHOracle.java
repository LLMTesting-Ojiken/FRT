...
public class SQLiteLineEHOracle implements TestOracle<SQLite3GlobalState> {

    private final SQLite3GlobalState globalState;
    private final Random random = new Random();

    public SQLiteLineEHOracle(SQLite3GlobalState globalState) {
    ...
    private boolean isMismatch(Throwable t1, Throwable t2) {
        ...
        return !t1.getClass().equals(t2.getClass())
                ...
                || getSQLiteErrorCode(t1) != getSQLiteErrorCode(t2);
    }
    ...
    private int getSQLiteErrorCode(Throwable t) {
        return (t instanceof SQLiteException) ? ((SQLiteException) t).getResultCode().code : -1;
    }
    ...
    private String safeMessageDetailed(Throwable t) {
        ...
        sb.append("SQLiteErrorCode: ").append(getSQLiteErrorCode(t)).append("\n");
        Throwable cause = t.getCause();
        if (cause != null) {
            sb.append("Cause Class: ").append(cause.getClass().getName()).append("\n");
            sb.append("Cause Message: ").append(cause.getMessage()).append("\n");
            sb.append("Cause SQLiteErrorCode: ").append(getSQLiteErrorCode(cause)).append("\n");
        }
        return sb.toString();
    }
    ...
    private String buildErrorMessage(String originalSql, Throwable origCause,
        ...
        if (origCause != null || complexCause != null) {
            ...
            sb.append("Original SQLiteCode: ").append(origCause != null ? getSQLiteErrorCode(origCause) : "<null>").append("\n");
            sb.append("Complex SQLiteCode: ").append(complexCause != null ? getSQLiteErrorCode(complexCause) : "<null>").append("\n");
            ...
            sb.append("Same SQLiteCode: ").append(origCause != null && complexCause != null
                    ? getSQLiteErrorCode(origCause) == getSQLiteErrorCode(complexCause) : "N/A").append("\n");
        }
    ...
    private String dumpTable(SQLite3GlobalState state, SQLite3Schema.SQLite3Table table) {
        StringBuilder sb = new StringBuilder();
    ...
    private String generateEnhancedComplexEquivalent(String originalSql) {
        ...
        for (int i = 0; i < funcLayers; i++) {
            ...
            switch (choice) {
                ...
                case 4: sql = "SELECT IIF(1=1,val,val) AS val FROM (" + sql + ") AS f" + i; break;
                case 5: sql = "SELECT CAST(val AS REAL) AS val FROM (" + sql + ") AS f" + i; break;
    ...
    /* ==================== 异常等价逐层脱壳 ==================== */
    private String tryUnwrapLayers(String complexSql, SQLite3GlobalState state, Throwable targetCause) {
    ...
    /* ==================== 异常等价判断 ==================== */
    private boolean isExceptionEquivalent(String candidateSql, Throwable targetCause, SQLite3GlobalState state) {
